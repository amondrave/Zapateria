...
public class SuelaDAOMySql implements SuelaDAO {
    ...
    final String GETONE = "SELECT referencia, nombre FROM suela WHERE referencia = ?";
    final String GETREF = "SELECT referencia FROM suela WHERE nombre = ?";
    ...
    public String obtenerSuela(String nombre) throws DAOException{
     PreparedStatement stat = null;
        ResultSet rs = null;
        String suela = null;
        try {
            stat = (PreparedStatement) conn.prepareStatement(GETREF);
            stat.setString(1, nombre);
            rs = stat.executeQuery();
            if (rs.next()) {
                suela = convertirRef(rs);
            } else {
                throw new DAOException("No existe ese registro en la base");
            }
        } catch (SQLException ex) {
    ...
    public String convertirRef(ResultSet rs)throws DAOException{
     String referencia = "";
     try{
       referencia = rs.getString("referencia");
     }catch(SQLException ex){
     throw new DAOException("No se encuentra esa referencia",ex);
        }
     return referencia;
    }
}
